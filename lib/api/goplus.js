...
async function fetchWithTimeout(url, options = {}, ms = FETCH_TIMEOUT_MS) {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw new Error(`GoPlus timeout after ${ms}ms`);
    }
    throw error;
  }
}
...
async function fetchWithRetry(url, options, retries = MAX_RETRIES) {
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      console.log(`GoPlus attempt ${i + 1}/${retries + 1} failed:`, error.message);
...
export async function checkTokenSecurity(tokenAddress, chain = 'base') {
  ...
  try {
    ...
    const isVerified = result.is_open_source === '1';
    ...
    const top10HolderRate = parseFloat(result.top10_holder_rate) * 100 || 0;
    ...
    // Return null instead of throwing, so other services can still work
    return null;
  }
}
