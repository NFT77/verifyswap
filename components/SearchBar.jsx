          ...
          <button
            ...
            className={`absolute inset-y-0 right-2 my-1.5 px-5 rounded-xl font-medium transition-all duration-200 ${
              query.trim() && !isLoading
                ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white hover:opacity-90 hover:scale-[1.02] active:scale-[0.98]'
      ...
      <AnimatePresence>
        {isFocused && recentSearches.length > 0 && !isLoading && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute z-10 mt-2 w-full max-w-2xl bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-xl"
          >
            <div className="p-2">
              ...
              {recentSearches.slice(0, 5).map((item, idx) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 hover:bg-white/10 transition-colors rounded-lg flex items-center gap-3 group"
                >
